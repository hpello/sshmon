...
const formatDuration = (secs: number): React.ReactNode => {
  if (secs < 60) { return <>&lt;{thinnbsp}1{thinnbsp}min</> }

  let seconds = secs
  const weeks = Math.floor(seconds / (7 * 24 * 60 * 60))
  if (weeks > 0) { seconds -= weeks * 7 * 24 * 60 * 60 }

  const days = Math.floor(seconds / (24 * 60 * 60))
  if (days > 0) { seconds -= days * 24 * 60 * 60 }
  ...
  return (<>
    {weeks > 0 ? <>{weeks}{thinnbsp}w </> : null}
    {days > 0 ? <>{days}{thinnbsp}d </> : null}
    {hours > 0 ? <>{hours}{thinnbsp}h </> : null}
    {minutes}{thinnbsp}min
  </>)
}
...
const formatMemory = (bytes: number): React.ReactNode => {
  ...
  if (Math.floor(g * 10) > 0) {
    return <>{(bytes / (2 ** 30)).toFixed(2)}{thinnbsp}GB</>
  }

  const m = bytes / (2 ** 20)
  if (Math.floor(m * 10) > 0) {
    return <>{(bytes / (2 ** 20)).toFixed(2)}{thinnbsp}MB</>
  }

  return <>{(bytes / (2 ** 10)).toFixed(2)}{thinnbsp}kB</>
}

const formatPercentage = (value: number): React.ReactNode => {
  return <>{value.toFixed(2)}{thinnbsp}%</>
}

const makeSystemSummary = (info: SystemInfoType): React.ReactNode => {
  return (<>
    {info.platform} ({info.arch}){largesp}|{largesp}
    {info.totalCPUs}{thinnbsp}CPU{info.totalCPUs > 1 ? 's' : ''}{largesp}|{largesp}
    ...
    return (
      <div className="panel">
        <div className="panel-block">
          <table className="table is-fullwidth is-marginless is-narrow is-striped is-hoverable" style={{ border: 'none' }}>
            <tbody>
              <tr>
...
export default connect<StateProps, {}, OwnProps, State>(mapStateToProps)(SystemInfo)
