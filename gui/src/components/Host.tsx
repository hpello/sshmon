...
import { AutoconnectState, AutoconnectSubState } from '../../../server/src/autoconnect/reducer'
...
const makeStatusText = (state: HostSubState, autoconnect: AutoconnectSubState) => {
  if (autoconnect.autoretryId && state.status !== 'connecting') {
    if (autoconnect.timeout === 0) { return 'Reconnecting' }
    return `Reconnecting in ${autoconnect.timeout / 1000}s…`
  }
  if (state.status === 'connecting' && state.reason === CONNECT_REASON_AUTORETRY) {
    return 'Reconnecting'
  }
...
const makeStatus = (state: HostSubState, autoconnect: AutoconnectSubState) => {
  const colorClass = (state.status !== 'connecting' && autoconnect.autoretryId) ? 'has-text-info' : makeColorClass(state.status)
...
class Host extends React.Component<Props, ComponentState> {
  ...
  computeContentHeight() {
    if (this.panelBlock === null) { return }
  ...
  componentDidMount() { this.computeContentHeight() }
  componentDidUpdate() { this.computeContentHeight() }
  ...
  render() {
    if (!this.props.state || !this.props.autoconnect) { return null }
    ...
    return (<>
      <div className="panel">
        <div
          ...
          style={{ display: 'block', height: 'auto', borderBottomLeftRadius: 0, borderBottomRightRadius: 0 } /* reset styles added by 'button' */}
            ...
            <div className="column" style={{ marginRight: '0.5rem' }}>
              <div className="columns is-mobile is-gapless is-multiline is-vcentered">
                <div className="column is-narrow" style={{ marginRight: '1rem' }}>
                  <div className="title is-5 has-text-weight-semibold">{config.label || id}</div>
                </div>
                <div className="column">
                  <div className="tags">
                    {this.props.autoconnect.config.start ? <div className="tag" title="autostart">start</div> : null}
                    {this.props.autoconnect.config.retry ? <div className="tag" title="autoretry">retry</div> : null}
                  </div>
                </div>
              </div>
            </div>
        ...
        >
          <table className="table is-fullwidth is-marginless is-narrow is-hoverable">
            <tbody>
              ...
              <tr>
                <td style={{ border: 'none' }}>
                  <ForwardingCreate id={this.props.id} apiClient={this.props.apiClient} />
                </td>
              </tr>
            </tbody>
          </table>
        </div>
        <div className="panel-block" style={{ display: 'block' }}> {/* https://github.com/jgthms/bulma/issues/1563 */}
          <div className="level is-mobile">
            <div className="level-left" />
            <div className="level-right">
              <div className="buttons">
                <button disabled={!['disconnected', 'error'].includes(state.status)} className="button is-small" onClick={() => this.setState({ ...this.state, editIsActive: true })}>Edit</button>
                {['disconnected', 'error'].includes(state.status) ? (
                  <button className="button is-primary is-small" onClick={() => apiClient.hostConnect({ id })}>Connect</button>
                ) : (
                  <button className="button is-primary is-small is-outlined" onClick={() => apiClient.hostDisconnect({ id })}>Disconnect</button>
                )}
              </div>
            </div>
          </div>
...
const mapStateToProps = (state: State, ownProps: OwnProps): Props => ({
  ...
  autoconnect: state.api.state.autoconnects.find(x => x.id === ownProps.id) || null
})
