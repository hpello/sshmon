...
import type { ForwardingConfig, ForwardingSpec } from '@/server/forward/types'
...
class ForwardingForm extends React.Component<Props, ComponentState> {
  ...
  checkIsValidFwdId(): string {
    ...
    if (this.state.fwdId === '') {
      return 'Forwarding ID is required.'
    }
    if (
      this.props.forwardings
        .filter((x) => x.id === this.props.id)
        .map((x) => x.fwdId)
        .includes(this.state.fwdId)
    ) {
      return 'Forwarding ID is already taken.'
    }
  ...
  getErrors(): string[] {
    return [this.checkIsValidFwdId()].filter((x) => x)
  }

  submit() {
    const { id, fwdId, label, type, bind, target, autostart, autoretry } =
      this.state
  ...
  delete() {
    if (this.props.fwdId === null) {
      return
    }
    this.props.apiClient.forwardingDelete({
      id: this.props.id,
  ...
  onChangeFwdId(event: React.ChangeEvent<{ value: string }>) {
    this.setState({ ...this.state, fwdId: event.target.value })
  }
  ...
  onChangeType(event: React.ChangeEvent<{ value: string }>) {
    this.setState({
      ...this.state,
      type: event.target.value as fwdTypes,
      bind: '',
      target: '',
    })
  }
  onChangeBind(event: React.ChangeEvent<{ value: string }>) {
    this.setState({ ...this.state, bind: event.target.value })
  }
  onChangeTarget(event: React.ChangeEvent<{ value: string }>) {
                ...
                <div className="field">
                  <div className="control">
                    <input
                      className="input"
                      type="text"
                      placeholder="my-cool-forwarding"
                      value={this.state.fwdId}
                      onChange={this.onChangeFwdId.bind(this)}
                ...
                <div className="field">
                  <div className="control">
                    <input
                      className="input"
                      type="text"
                      placeholder="[address:]port, /path/to/sock&hellip;"
                      value={this.state.bind}
                      onChange={this.onChangeBind.bind(this)}
                      disabled={this.state.type === fwdTypes.http}
                      style={{ fontFamily: 'monospace' }}
                    />
                  </div>
                  <div className="help">
                ...
                <div className="field">
                  <div className="control">
                    <input
                      ...
                      disabled={this.state.type === fwdTypes.dynamic}
                      style={{ fontFamily: 'monospace' }}
                    />
                  </div>
                  <div className="help">
