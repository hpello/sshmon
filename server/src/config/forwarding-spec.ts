...
export const serializeForwardingSpec = (spec: ForwardingSpec): string => {
  switch (spec.type) {
  case fwdTypes.dynamic: return `D ${spec.bind}`
  case fwdTypes.local: return `L ${spec.bind} ${spec.target}`
  case fwdTypes.remote: return `R ${spec.bind} ${spec.target}`
  case fwdTypes.http: return `H ${spec.target}`
  }
}
