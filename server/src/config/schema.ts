...
const replaceIfNull = (schema: any, defoult: any) =>
...
const forwarding = Joi.alternatives().try(forwardingObject, Joi.string())
...
const host = Joi.object({
  ...
  forward: Joi.array()
    .items(Joi.object().pattern(/.*/, forwarding).length(1))
...
export const configSchema = Joi.object({
  hosts: Joi.array()
    .items(Joi.object().pattern(/.*/, replaceIfNull(host, {})).length(1))
    .unique((a, b) => Object.keys(a)[0] === Object.keys(b)[0]),
  config,
