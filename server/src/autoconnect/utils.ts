...
const autoconnectExist = (state: State, id: string): boolean => some(state.autoconnects, { id })
...
const cancelAutoconnects = (prevState: State, state: State, dispatch: Dispatch) => {
  state.autoconnects.map(x => x.id)
