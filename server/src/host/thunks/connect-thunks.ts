...
const extractConfigControlPath = (hostConfig: HostConfig): string | null => {
  ...
  const key = Object.keys(sshConfig).find(k => k.toLowerCase() === 'controlpath')
  return key ? sshConfig[key] : null
}

const accessAsync = promisify(access)
const fileExists = (path: string): Promise<boolean> => accessAsync(path).then(() => true).catch(() => false)

export const hostConnect = (id: string, reason: string): AsyncThunkAction => async (dispatch: Dispatch, getState: GetState) => {
  const state = getState()
  const host = find(state.hosts, { id })
  if (!host) { throw new ErrorWithCode(404, `host not found: ${id}`) }

  if (!['disconnected', 'error'].includes(host.state.status)) { log.info(`host not down: ${id}`); return }

  const controlPath = extractConfigControlPath(host.config) || await makeTmpPath(__filename)(id)
  ...
  process.on('exit', (code) => {
    watcher.close()
    dispatch((code === 0 || code === null) ? actions.hostStateDisconnected(id) : actions.hostStateError(id))
  })
}

export const hostDisconnect = (id: string, reason: string): AsyncThunkAction => (dispatch: Dispatch, getState: GetState) => {
  const state = getState()
  const host = find(state.hosts, { id })
  if (!host) { throw new ErrorWithCode(404, `host not found: ${id}`) }

  if (!['connecting', 'connected'].includes(host.state.status)) { log.info('host is not connected:', id); return Promise.resolve() }
  if (host.state.pid === null) { throw new ErrorWithCode(500, `bad state: ${id} (${JSON.stringify(host.state)})`) }
