...
const makeNextTimeout = (autoforward: AutoforwardState): number => {
  if (autoforward.state.timeout === 0) { return MIN_TIMEOUT_MS }

  const timeout = autoforward.state.timeout * NEXT_TIMEOUT_FACTOR
  if (timeout > MAX_TIMEOUT_MS) { return MAX_TIMEOUT_MS }
...
export const autoretrySpawn = (id: string, fwdId: string) => async (dispatch: Dispatch, getState: GetState) => {
  ...
  dispatch(actions.autoforwardLaunch(id, fwdId, autoretryId, numRetries, timeout))

  setTimeout(() => {
    const a = find(getState().autoforwards, { id, fwdId })
    if (a && a.state.autoretryId === autoretryId) {
      dispatch(forwardTunks.forwardingConnect(id, fwdId, FORWARD_REASON_AUTORETRY))
    }
  }, timeout)
}
