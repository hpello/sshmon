...
const handleRequest = (server: SimpleProxyServer) => (req: IncomingMessage, res: ServerResponse) => {
  ...
  if (pathPrefix !== '/') {
    if (!url.startsWith(`${pathPrefix}/`)) {
      res.writeHead(302, { Location: `${pathPrefix.slice(pathPrefix.lastIndexOf('/') + 1)}/` })
      res.end()
      return
    }
  ...
  proxy.proxyServer.web(req, res, (err) => {
    if (err.code === 'ECONNRESET') {
      ...
      res.end(JSON.stringify({ code: 'BadGatewayError', message: 'Target could not be read', error: err }))
      return
    }

    res.writeHead(500, { 'Content-Type': 'application/json' })
    res.end(JSON.stringify({ code: 'InternalServerError', message: 'Internal server error', error: err }))
  })
}

const handleUpgrade = (server: SimpleProxyServer) => (req: IncomingMessage, socket: Socket, head: Buffer) => {
  const { proxies } = server
  const url = req.url as string // see IncomingMessage
...
export class SimpleProxyServer {
  ...
  addTarget(pathPrefix: string, target: ProxyTarget) {
    ...
    this.proxies = this.proxies
      ...
      .sort((a, b) => { // sort path prefix descending to ensure path processing order later
